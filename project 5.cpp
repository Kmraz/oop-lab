...
int main()
{
    Mess_Owner *memberinfo[5]={
        new Mess_Owner("Alif",1500),
        new Mess_Owner("Rajib",1200),
        new Mess_Owner("Raz",3000),
        new Mess_Owner("Zim",900),
            };
    memberinfo[0]->get_meal(1,0,1,0,1,1,1,1,1 );
    memberinfo[1]->get_meal(1,0,1,0,1,1,1,1,1 );
    memberinfo[2]->get_meal(1,0,1,0,1,1,1,1,1 );
    memberinfo[3]->get_meal(1,0,1,0,1,1,1,1,1 );

    Mess_Owner Hridoy;
    Hridoy.get_totalcost(900);
    int sum=0;
    float  mealrate;
    for(int i=0;i<4;i++)
    {
    ...
    mealrate=Hridoy.get_mealrate(sum);
    for(int i=0;i<4;i++)
       {
